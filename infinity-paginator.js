 ...
 */
export function InfinityPaginator(options) {
    ...
    if (typeof this._options.edgePixels === 'object') {
    ...
    if (typeof this._options.loadEdgePixels === 'object') {
 ...
 */
InfinityPaginator.prototype._scrollTo = function(position, duration, options) {
 ...
 * @private
 */
InfinityPaginator.prototype._initState = function(options) {
 ...
 * @param {Object} options
 */
InfinityPaginator.prototype.reinit = function(options) {
 ...
 * @param {Object} options
 */
InfinityPaginator.prototype.load = function infPaginatorLoadPrototype(options) {
    var req;
 ...
 */
InfinityPaginator.prototype.cancel = function infPaginatorCancelPrototype() {
 ...
 */
InfinityPaginator.prototype.loadPage = function infPaginatorLoadPagePrototype(options) {
 ...
 * @param {Object} res
 */
InfinityPaginator.prototype.didFailLoadPage = function infPaginatorDidFailLoadPagePrototype(options, res) {
 ...
 * @param {Object} res
 */
InfinityPaginator.prototype.didLoadPage = function infPaginatorDidLoadPagePrototype(options, res) {
...
InfinityPaginator.prototype.withSavedScroll = function infPaginatorWithSavedScrollPrototype(action) {
 ...
 * @param {Object} res
 */
InfinityPaginator.prototype.renderPage = function infPaginatorRenderPagePrototype(options, res) {
    var $container = $(this._options.container === window ? document.body : this._options.container);
    var $html;
    ...
    if (this._options.template) {
        ...
        $html = '<div class="' + this._options.pageClass + '"' +
            ' id="page-' + options.page + '" data-page="' + options.page + '">' + $html.html() + '</div>';
        $html = $($html);
    }
 ...
 */
InfinityPaginator.prototype.nextPage = function infPaginatorNextPagePrototype(options) {
 ...
 */
InfinityPaginator.prototype.prevPage = function infPaginatorPrevPagePrototype(options) {
    var page = this._state.currentPage - 1;
    if (page > 0) {
        if (this._state.pageCache[page]) {
            this.renderPage({
 ...
 * @return {*}
 */
InfinityPaginator.prototype.nearTheEdge = function(options) {
    var toTop,
        toRight,
        toBottom,
        toLeft;
    ...
    if (options.full) { // XXX always true
        if (options.load) { // XXX always true
            return {
                load: {
 ...
 */
InfinityPaginator.prototype.navigate = function(itemData) {
    ...
    if (this._options.selfNavigation && 
...
InfinityPaginator.prototype.pause = function infPaginatorPausePrototype() {
    this._state.isPaused = true;
};

InfinityPaginator.prototype.resume = function infPaginatorResumePrototype() {
 ...
 * @return {InfinityPaginator}
 */
InfinityPaginator.prototype.scrollToPage = function(pageNumber, animated) {
 ...
 */
InfinityPaginator.prototype.preloadPages = function() {
    this.prevPage();
    this.nextPage();
    // Chaining.
    return this;
 ...
 */
InfinityPaginator.prototype.addEvent = function(element, type, handler) {
    if (window.jQuery !== undefined) {
 ...
 * @param {Function} handler
 */
InfinityPaginator.prototype.removeEvent = function(element, type, handler) {
    if (window.jQuery !== undefined) {
...
InfinityPaginator.prototype.onScroll = function(e) {
    var self = this;
 ...
 * @param {Object} options
 */
InfinityPaginator.prototype.bind = function(container, options) {
    // Adds keyboard event listener to handle space, page up, etc.
    this.addEvent(window, 'keydown', function(e) {
        if ((container && container.tagName && !container.closest('body').length) ||
              (!this._options.holder || !this._options.holder.length)) {
            this.unbind();
            return;
        }
        ...
        if (
            (e.shiftKey && e.keyCode === 32) /*Shift+Space*/ ||
                e.keyCode === 38 /*Up*/) {
            this.prevPage();
        }
    }.bind(this));
 ...
 * @param {Element|jQuery} container
 */
InfinityPaginator.prototype.unbind = function(container) {
 ...
 */
InfinityPaginator.prototype.trigger = function(eventName/*, extraParams*/) {
 ...
 * @param {Function} func
 */
InfinityPaginator.prototype.on = function(eventName, func) {
 ...
 * @param {Function} func
 */
InfinityPaginator.prototype.off = function(eventName, func) {
